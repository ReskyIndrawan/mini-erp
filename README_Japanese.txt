...
• 累計 - 全データの累計合計を表示するフィールド（自動入力）
• № - 月ごとの連番フィールド（毎月1からリセット、自動入力）
...
【データを削除：】
1. 【単一削除】 - プレビューテーブルのデータ行をクリックし、削除ボタンをクリックしてデータを削除（確認ダイアログが表示されます）
2. 【複数削除】
   - Ctrlキー（Windows/Linux）またはCmdキー（Mac）を押しながらクリックして、複数の行をランダムに選択
   - またはShiftキーを押しながらクリックして、範囲選択
   - 削除ボタンをクリックして選択したすべてのデータを削除（削除するデータ数が表示された確認ダイアログが表示されます）
...
• 削除：
  - 確認ダイアログを表示（単一削除の場合）または削除するデータ数を表示した確認ダイアログ（複数削除の場合）
  - Excelファイルから単一または複数のデータを削除
  - プレビュー表示を更新
  - 入力フォームをクリア
...
【複数削除】
アプリケーションは複数のデータの一括削除をサポートするようになりました：
• Ctrlキー（Windows/Linux）またはCmdキー（Mac）+クリックで複数の行をランダムに選択
• Shiftキー+クリックで範囲選択
• 削除ボタンで選択したすべてのデータを一括削除
• 確認ダイアログには削除されるデータ数が表示されます
...
【自動番号付け】
システムには2種類の自動番号付けがあります：
• 累計 - Excelファイル内の全データに基づく連番（1, 2, 3, ...）
• № - 発生月（Occurrence Month）に基づき、毎月1からリセットされる月ごとの連番
